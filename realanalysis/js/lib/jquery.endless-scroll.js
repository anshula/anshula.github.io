...
/*
  ...
  lastPage          string          either 'false' (in which case you can keep scrolling down) or a string with a number (e.g.) '3'
                                    to indicate when you bottom out and can't scroll anymore
...
EndlessScroll = (function() {
  ...
  function EndlessScroll(scope, options) {
    ...
    // set up the global variable to point to this
    myEndlessScroll = this;
  ...
  EndlessScroll.prototype.wrapContainer = function(target) {
    if (this.innerWrap.length === 0) {
      return this.innerWrap = $("#images").wrapInner('<div class="endless_scroll_content" data-page="0" />').wrapInner('<div class="endless_scroll_inner_wrap" />').find('.endless_scroll_inner_wrap');
    }
  };

  EndlessScroll.prototype.isOutOfBounds = function() {
    var _this = this;
    if(this.scrollDirection=="prev" && $(".comicpage:first").attr("data-comicpage")=="1") {
      return true;
    }
    else if(this.scrollDirection=="next" && $(".comicpage:last").attr("data-comicpage")==this.options.lastPage) {
      return true;
    }
    else {
      return false;
  ...
  EndlessScroll.prototype.resetEndlessScroll = function() {
    this.pagesStack = [0];
  };
  ...
  EndlessScroll.prototype.cleanUpPagesWhenNecessary = function() {
    ...
    if(this.isOutOfBounds()) {
      // find the data type associated with it
      // add it to the page sequence the very first time!
      pagenumberof_firstimage = $(".comicpage:first").attr("data-page");
      pagenumberof_lastimage =  $(".comicpage:last").attr("data-page");
      if(this.scrollDirection == 'prev' && this.pagesStack[0] == pagenumberof_firstimage) { // only true if we've already scrolled up here before
        // so it's not necessary to add this page to the sequence
        return;
      }
      else if (this.scrollDirection == 'next' && this.pagesStack[this.pagesStack.length-1] == pagenumberof_lastimage) {// only true if we've already scrolled down here before
        // so it's not necessary to add this page to the sequence
        return;
      }
  ...
  EndlessScroll.prototype.removePage = function(page) {
    $("div[data-page='" + page + "']", $("body")).remove();
  };
